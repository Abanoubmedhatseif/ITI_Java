...
public class InMemoryWorldDao implements WorldDao {
    ...
    public static void main(String args[]) {
        ...
        Map<Country,City> HighestPopulatedCityInCountryObject = allCountries.entrySet().stream().map(
                        entry -> Map.entry( entry.getValue() ,
                                entry.getValue().getCities().stream()
                                .max(Comparator.comparingInt(City::getPopulation))
                                .orElse(new City()) // null didnt work !!!
                        ))
                .collect(Collectors.toMap(Map.Entry::getKey,Map.Entry::getValue));
//        HighestPopulatedCityInCountryObject.forEach((key,value) ->
//                System.out.println(key.getName() + ":" + value.getName()));




        Map<String,String> HighestPopulatedCityInCountry = allCountries.entrySet().stream().map(
                   entry -> Map.entry( entry.getValue().getName() ,
                           entry.getValue().getCities().stream()
                           .max(Comparator.comparingInt(City::getPopulation))
                           .map(City::getName)
                           .orElse("")
...
//        HighestPopulatedCityInCountry.forEach((key,value) ->
//                System.out.println(key + ":" + value));




        Map<String, Optional<City>> mostPopulatedCityInContinent = HighestPopulatedCityInCountryObject.entrySet().stream()
                .collect(Collectors.groupingBy(
                        entry -> entry.getKey().getContinent(),
                        Collectors.mapping( entry -> entry.getValue() ,
                        Collectors.maxBy(Comparator.comparingInt(City::getPopulation))
                        )
        ));
//        mostPopulatedCityInContinent.forEach((key,value) ->
//                System.out.println(key + ":" + value.map(City::getName).orElse("No city")));

        Map<Country,City> CapitalsOfTheWorld = allCountries.entrySet().stream().map(
                        entry -> Map.entry( entry.getValue() ,
                                entry.getValue().getCities().stream()
                                        .filter(city -> city.getId() == entry.getValue().getCapital())
                                        .findFirst()
                                        .orElse(new City())
                        ))
                .collect(Collectors.toMap(Map.Entry::getKey,Map.Entry::getValue));
//        CapitalsOfTheWorld.forEach((key,value) ->
//        System.out.println(key.getName() + ":" + value.getName()));



        Optional<Map.Entry<Country, City>> mostPopulatedCapital = CapitalsOfTheWorld.entrySet().stream()
                .reduce((entry1, entry2) ->
                        entry1.getValue().getPopulation() > entry2.getValue().getPopulation() ? entry1 : entry2);

        // if present added to handle output from optional if null
        mostPopulatedCapital.ifPresent(entry ->
        System.out.println("Most Populated Capital: " + entry.getValue().getName() + " - Population: " + entry.getValue().getPopulation()));
